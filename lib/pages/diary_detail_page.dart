...
class _DiaryDetailPageState extends State<DiaryDetailPage> {
  ...
  Future<void> _loadContent() async {
    final summary = await _storageService.readSummary(widget.entry.folderPath);
    final transcript = await _storageService.readTranscript(widget.entry.folderPath);
    if (mounted) {
      setState(() {
        _summary = summary;
  ...
  Future<void> _deleteDiary() async {
    ...
    if (confirmed == true && mounted) {
      await _storageService.deleteEntry(widget.entry.id, widget.entry.folderPath);
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.delete_outline), onPressed: _deleteDiary),
        ],
      ),
      body: _loading
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (audioExists)
                    AudioPlayerBar(playerService: _playerService, audioFilePath: audioPath),
                  const SizedBox(height: 16),
