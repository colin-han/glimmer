...
class _TagManagementPageState extends State<TagManagementPage> {
      ...
      if (mounted) {
        ...
        if (confirmedDiaryIds != null && confirmedDiaryIds.isNotEmpty) {
          try {
            ...
            for (final entry in confirmedEntries) {
              ...
              try {
                if (await _storageService.hasLlmResult(entry.folderPath)) {
                  final llmData = await _storageService.readLlmResult(
                    entry.folderPath,
                  );
            ...
            final prompt = await _llmService.generateMatchPrompt(
              name,
              diarySummaries,
            );
  ...
  Future<List<String>?> _showRecommendations(Tag tag) async {
    ...
    final recommendations = await _llmService.recommendDiariesForTag(
      tag.name,
      diarySummaries,
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const AppTitle(title: '标签管理')),
      body: Stack(
        children: [
          _loading
